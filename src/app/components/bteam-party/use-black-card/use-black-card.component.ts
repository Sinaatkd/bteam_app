...
@Component({
  selector: 'app-use-black-card',
  templateUrl: './use-black-card.component.html',
  styleUrls: ['./use-black-card.component.scss'],
})
export class UseBlackCardComponent implements OnInit {

  @Input('cardCode') cardCode: string;
  @Output() getGiftResponseEmitter = new EventEmitter();
  userPhonenumberAndFullname: string;
  selectedGift: 'free-transaction' | 'cash';
  isConfirmInfo = false;
  ...
  onContinueButtonClicked() {
    ...
    this.bteamPartyService.useGift('black-b', this.selectedGift).subscribe(res => {
